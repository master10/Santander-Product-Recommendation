...
def monthly_new_prod_users(df,fecha_date_prev,fecha_date_cur):
    ...
    Target_y = pd.DataFrame(Target_y.groupby(['ncodpers'])[products].sum())
    Target_y.reset_index(level=0, inplace=True)
    return(Target_y)

user_new_prod_0416 = monthly_new_prod_users(df_train,'2016-03-28','2016-04-28')
#user_new_prod_0516 = monthly_new_prod_users(df_train,'2016-04-28','2016-05-28')
user_new_prod_0615 = monthly_new_prod_users(df_train,'2015-05-28','2015-06-28')
#user_new_prod_0715 = monthly_new_prod_users(df_train,'2015-06-28','2015-07-28')
...
def training_set(df_train,df,date):
    ...
    df_train_new = df_train_new.drop(products,axis=1)
    df_train_new = pd.merge(df_train_new,df,how='inner',on=['ncodpers'])
    cols = df_train_new[products].columns
    temp = df_train_new[products].apply(lambda x: x > 0)
    df_train_new['target_combo'] = temp.apply(lambda x: list(cols[x.values]), axis=1) 
    df_train_new['target_combo_len'] = df_train_new.target_combo.str.len()
    df_train_new['target_combo'] = df_train_new['target_combo'].astype(list)
    return(df_train_new)

df_train_new_0416 = training_set(df_train,user_new_prod_0416,'2016-04-28')
#df_train_new_0516 = training_set(df_train,user_new_prod_0516,'2016-05-28')
df_train_new_0615 = training_set(df_train,user_new_prod_0615,'2015-06-28')
#df_train_new_0715 = training_set(df_train,user_new_prod_0715,'2015-07-28')

def reduce_target(df):
    df_temp = pd.DataFrame(df.target_combo.astype(str).value_counts())
    df_temp = df_temp.ix[df_temp[0]>1,:]
    df_temp.reset_index(level=0, inplace=True)
    df_temp.columns = ['target_combo','cnt']
    for i in range(len(df_temp.target_combo)):
        df_temp.target_combo[i] = ast.literal_eval(df_temp.target_combo[i])
    df['target_combo_temp'] = df['target_combo'].astype(str)
    df_temp['target_combo_temp'] = df_temp['target_combo'].astype(str)
    df = pd.merge(df,df_temp[['target_combo_temp']],how='inner',on=['target_combo_temp'])
    df = df.drop(['target_combo_temp'],axis=1)
    return(df)
    
df_train_new_0416 = reduce_target(df_train_new_0416)
df_train_new_0615 = reduce_target(df_train_new_0615)
#    pd.concat([pd.Series(row['cnt'], row['target_combo'].split(',')) for _, row in df_temp1.iterrows()]).reset_index()
#    df_temp2 = pd.DataFrame(df_train_new_0615.target_combo[df_train_new_0615.target_combo.str.len()==1].astype(str).value_counts())

#months_before_0615 = ['2015-01-28','2015-02-28','2015-03-28','2015-04-28','2015-05-28']
#months_before_0416 = ['2015-01-28','2015-02-28','2015-03-28','2015-04-28','2015-05-28',
#                      '2015-06-28','2015-07-28','2015-08-28','2015-09-28','2015-10-28',
#                      '2015-11-28','2015-12-28','2016-01-28','2016-02-28','2016-03-28']
#months_before_0616 = ['2015-01-28','2015-02-28','2015-03-28','2015-04-28','2015-05-28',
#                      '2015-06-28','2015-07-28','2015-08-28','2015-09-28','2015-10-28',
#                      '2015-11-28','2015-12-28','2016-01-28','2016-02-28','2016-03-28',
...
#df_train_new_0516 = data_prep_dmg(df_train_new_0516)
df_train_new_0615 = data_prep_dmg(df_train_new_0615)
#df_train_new_0715 = data_prep_dmg(df_train_new_0715)
...
#df_train_new_0516 = add_prev_prod_purchase_info(df_train,df_train_new_0516,['2015-12-28','2016-01-28','2016-02-28','2016-03-28','2016-04-28'])
...
#df_train_new_0715 = add_prev_prod_purchase_info(df_train,df_train_new_0715,['2015-02-28','2015-03-28','2015-04-28','2015-05-28','2015-06-28'])
...
#df_train_new_0516 = add_last5_prod_purchase_info(df_train,df_train_new_0516,['2015-12-28','2016-01-28','2016-02-28','2016-03-28','2016-04-28'])
...
#df_train_new_0715 = add_last5_prod_purchase_info(df_train,df_train_new_0715,['2015-02-28','2015-03-28','2015-04-28','2015-05-28','2015-06-28'])
...
#df_train_new_0516 = add_last5_unique_prod_info(df_train,df_train_new_0516,['2015-12-28','2016-01-28','2016-02-28','2016-03-28','2016-04-28'])
...
#df_train_new_0715 = add_last5_unique_prod_info(df_train,df_train_new_0715,['2015-02-28','2015-03-28','2015-04-28','2015-05-28','2015-06-28'])
...
#df_train_new_0516 = add_monthly_new_prod(df_train,df_train_new_0516,['2015-12-28','2016-01-28','2016-02-28','2016-03-28','2016-04-28'])
df_train_new_0615 = add_monthly_new_prod(df_train,df_train_new_0615,['2015-01-28','2015-02-28','2015-03-28','2015-04-28','2015-05-28'])
#df_train_new_0715 = add_monthly_new_prod(df_train,df_train_new_0715,['2015-02-28','2015-03-28','2015-04-28','2015-05-28','2015-06-28'])
df_sub = add_monthly_new_prod(df_train,df_sub,['2016-01-28','2016-02-28','2016-03-28','2016-04-28','2016-05-28'])

#df_train_new = df_train_new_0615.copy()
...
#df_train_new = pd.concat([df_train_new_0615,df_train_new_0715,df_train_new_0416, df_train_new_0516], axis=0,ignore_index=True)
#df_train_new = pd.concat([df_train_new_0615,df_train_new_0715,df_train_new_0416], axis=0,ignore_index=True)

#Input variables getting used in the model
cat_var = ['tenure_grp1','indresi','sexo','ind_empleado','tiprel_1mes','indext','channel_grp1','indfall','nomprov','segmento','age_grp1','income_grp1']
num_var = ['tenure','age','ind_actividad_cliente','indrel',
'ind_cco_fin_ult1_last0', 'ind_cder_fin_ult1_last0',
'ind_cno_fin_ult1_last0', 'ind_ctju_fin_ult1_last0','ind_ctma_fin_ult1_last0', 'ind_ctop_fin_ult1_last0',
'ind_ctpp_fin_ult1_last0', 'ind_dela_fin_ult1_last0',
...
'recent_activity_score_mnth','unique_prod']
...
#df_sub.loc[df_sub.channel_grp1 == 'KHQ','channel_grp1'] = 'KH'
...
target = pd.get_dummies(df_train_new.target_combo.astype(str)).as_matrix()

rms = rmsprop(lr=0.0005, rho=0.9, epsilon=1e-08, decay=0.0,clipnorm = 0.1)
# Model Building 
def keras_model():
    # create model
    model = Sequential()
    ...
    model.add(Dropout(0.5))
    model.add(Dense(len(target[0]), init='uniform', activation='softmax'))
    model.compile(loss='categorical_crossentropy', optimizer=rms, metrics=['accuracy'])
    return model

estimator = KerasClassifier(build_fn=keras_model, nb_epoch=41, batch_size=500, verbose=1)
np.random.seed(123)
estimator.fit(df_train_new_arr_normed, target,verbose=1,validation_split=0.3,show_accuracy=True)
predictions_train = estimator.predict_proba(df_train_new_arr_normed)
predictions_sub = estimator.predict_proba(df_sub_arr_normed)

pred_sub = pd.DataFrame(data=predictions_sub,columns=pd.get_dummies(df_train_new.target_combo.astype(str)).columns)
pred_sub['ncodpers'] = df_sub['ncodpers']

pred_train = pd.DataFrame(data=predictions_train,columns=pd.get_dummies(df_train_new.target_combo.astype(str)).columns)
pred_train['ncodpers'] = df_train_new['ncodpers']
pred_train['month'] = df_train_new['month']

pred_sub.to_pickle(wdir+'pred_sub_2.pkl')
pred_train.to_pickle(wdir+'pred_train_2.pkl')